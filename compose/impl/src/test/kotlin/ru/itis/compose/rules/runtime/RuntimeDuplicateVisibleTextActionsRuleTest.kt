...
class RuntimeDuplicateVisibleTextActionsRuleTest {
    ...
    fun `reports duplicate labels from clickable runtime actions`() {
        ...
        assertEquals(issues.single().message?.contains("Demo board"), true)
        assertEquals(issues.single().message?.contains("2"), true)
    }
    ...
    fun `uses content description as action label`() {
        ...
        assertEquals(1, issues.size)
        assertEquals(issues.single().message?.contains("Delete board"), true)
    }
