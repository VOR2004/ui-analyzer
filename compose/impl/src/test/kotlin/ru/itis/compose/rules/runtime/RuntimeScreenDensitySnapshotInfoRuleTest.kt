...
class RuntimeScreenDensitySnapshotInfoRuleTest {
    ...
    fun `reports compose runtime snapshot metadata`() {
        ...
        assertEquals(issues.single().message?.contains("BoardScreen"), true)
        assertEquals(issues.single().message?.contains("density=2.75"), true)
    }
    ...
    fun `infers android runtime snapshot size and orientation from bounds`() {
        ...
        assertEquals(1, issues.size)
        assertEquals(issues.single().message?.contains("1920px x 1080px"), true)
        assertEquals(issues.single().message?.contains("landscape"), true)
    }
